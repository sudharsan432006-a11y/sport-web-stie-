...
const RINGS_ROTATIONS = [...Array(3)].map(() => [Math.random() * Math.PI, Math.random() * Math.PI, 0]);
const PARTICLES_POSITIONS = [...Array(100)].map(() => [
  (Math.random() - 0.5) * 20,
  (Math.random() - 0.5) * 20,
  (Math.random() - 0.5) * 20
]);
  ...
  return (
    <>
      ...
      <group ref={groupRef}>
        ...
        {/* Floating rings */}
        {RINGS_ROTATIONS.map((rotation, i) => (
        ...
        {/* Particles */}
        <group>
            {PARTICLES_POSITIONS.map((position, i) => (
                <mesh
                    key={i}
                    position={position}
                >
