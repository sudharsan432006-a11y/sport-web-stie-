...
// Move random data generation outside component for purity and stability
const RING_ROTATIONS = [...Array(3)].map(() => [
  Math.random() * Math.PI,
  Math.random() * Math.PI,
  0
]);

const PARTICLE_POSITIONS = [...Array(100)].map(() => [
  (Math.random() - 0.5) * 20,
  (Math.random() - 0.5) * 20,
  (Math.random() - 0.5) * 20
]);
