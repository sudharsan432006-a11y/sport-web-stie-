...
// ✅ GOOD: Pre-calculate random values outside component to ensure purity
const RINGS_RANDOM = [...Array(3)].map(() => [Math.random() * Math.PI, Math.random() * Math.PI]);
const PARTICLES_RANDOM = [...Array(100)].map(() => [
    (Math.random() - 0.5) * 20,
    (Math.random() - 0.5) * 20,
    (Math.random() - 0.5) * 20
]);
  ...
  return (
    <>
      ...
      <group ref={groupRef}>
        ...
        {/* Floating rings */}
        {RINGS_RANDOM.map((rotation, i) => (
            <mesh key={i} rotation={[rotation[0], rotation[1], 0]}>
        ...
        {/* Particles */}
        <group>
            {PARTICLES_RANDOM.map((position, i) => (
                <mesh
                    key={i}
                    position={position}
                >
